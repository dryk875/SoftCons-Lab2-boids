...
def seqBoidToJson(boids: Seq[Boid]) =
  boids.map(b =>
    val jsonFields = Seq[Option[(String, ujson.Value)]](
      Some("x" -> ujson.Num(b.position.x)),
      Some("y" -> ujson.Num(b.position.y)),
      Some("vx" -> ujson.Num(b.velocity.x)),
      Some("vy" -> ujson.Num(b.velocity.y)),
      if b.size != Boids.DEFAULT_SIZE then Some("size" -> ujson.Num(b.size)) else None,
      if b.color != Boids.DEFAULT_COLOR then Some("rgb" -> ujson.Str(rgbToHex(b.color.getRGB()))) else None
    )
    ujson.Obj.from(jsonFields.filter(_.isDefined).map(_.get).toSeq)
  )
...
def generateTestCase(name: String, initialBoids: Seq[Boid], physics: Physics, steps: Int = 50) =
  val path = os.pwd / "src" / "test" / "json" / (name + ".json")
